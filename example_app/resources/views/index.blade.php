...
<div class="text-center mt-10">
  ...
  <form action="{{ route('saveTask') }}" method="POST" class="pt-10">
    @csrf
  ...
  <tbody>
    ...
    @foreach($tasks as $task)
      <tr>
        <td class="border-solid border-2 border-gray-200 pt-5 pb-5 text-left p-2">{{ $task->task_name }}</td>
        ...
        <a href="{{ route('deleteTask', $task->id) }}" class="bg-red-400 p-1 text-white rounded hover:bg-red-600 text-sm font-bolder">Delete</a>
        </td>
      </tr>
    @endforeach
    @endif
